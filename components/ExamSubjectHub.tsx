...
import { getExamById, getDefaultExamIdByTrack } from '../data/cetExamData';
import { getTrackSubjectBlueprintBySubject, getTrackSubjectBlueprints } from '../data/trackSubjectBlueprints';

const ExamSubjectHub: React.FC = () => {
  ...
  const blueprints = getTrackSubjectBlueprints(learnerProfile.targetCourse);
  ...
  const selectedSubject = useMemo(() => {
    ...
    return blueprints[0]?.subject || Subject.LogicalReasoning;
  }, [searchParams, blueprints]);

  const blueprint = getTrackSubjectBlueprintBySubject(learnerProfile.targetCourse, selectedSubject);

  return (
    <div className="space-y-5 md:space-y-6">
      <div className="bg-gradient-to-r from-indigo-600 to-violet-600 text-white p-5 md:p-7 rounded-2xl shadow-lg">
        <h2 className="text-xl md:text-3xl font-bold mb-2">{selectedExam.shortTitle} Subject Hub</h2>
        <p className="text-indigo-100 text-sm md:text-base max-w-3xl">
          Track-wise complete guidance: concept roadmap, module-level explanation, tips/tricks, and direct MCQ drills.
        </p>
      </div>
