...
import { 
  Scale, BookOpen, ChevronRight, ChevronLeft, CheckCircle, 
  XCircle, AlertCircle, Lightbulb, RotateCcw, Award, Filter
} from 'lucide-react';
import { LEGAL_REASONING_PASSAGES, LegalPassage, LegalReasoningQuestion } from '../data/legalReasoningData';

type DifficultyFilter = 'all' | 'easy' | 'medium' | 'hard';

export default function LegalReasoning() {
  const [selectedPassage, setSelectedPassage] = useState<LegalPassage | null>(null);
  ...
  const filteredPassages = difficultyFilter === 'all' 
    ? LEGAL_REASONING_PASSAGES 
    : LEGAL_REASONING_PASSAGES.filter(p => p.difficulty === difficultyFilter);

  const handleSelectPassage = (passage: LegalPassage) => {
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-gray-400 mt-1">Practice principle-based legal reasoning questions</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-gray-800 rounded-xl p-4">
              ...
              <p className="text-2xl font-bold text-white">{LEGAL_REASONING_PASSAGES.length}</p>
            </div>
            <div className="bg-gray-800 rounded-xl p-4">
              <p className="text-gray-400 text-sm">Total Questions</p>
              <p className="text-2xl font-bold text-white">
                {LEGAL_REASONING_PASSAGES.reduce((sum, p) => sum + p.questions.length, 0)}
              </p>
            </div>
            <div className="bg-gray-800 rounded-xl p-4">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-5 mb-6">
          <div className="flex items-center gap-2 text-blue-400 mb-3">
            <BookOpen size={20} />
            <h3 className="font-semibold">LEGAL PRINCIPLE</h3>
          </div>
          <p className="text-gray-300 leading-relaxed text-sm md:text-base">
            {selectedPassage.principle}
          </p>
        </div>
        ...
        <div className="bg-gray-800 rounded-xl p-5 mb-6">
          <div className="flex items-center gap-2 text-yellow-400 mb-3">
            <Scale size={20} />
            <h3 className="font-semibold">FACTUAL SITUATION</h3>
          </div>
          <p className="text-gray-300 leading-relaxed text-sm md:text-base">
            {selectedPassage.facts}
          </p>
        </div>
